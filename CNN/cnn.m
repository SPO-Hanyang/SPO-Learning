...
while(epoch<=n_epochs && looping)
    ...
    for minibatch_index = [1 : 1 : n_train_batches] 
        ...
        % convolution
        % temp_output0 = 24*24*20*500
        % maxpooling
        % output0 = 12*12*20*500
        layer0_input = reshape(x,image_shape0);
        [temp_output0] = convlayer(layer0_input,image_shape0,filter_shape0,conv_stride0,w0,b0);
        [output0,pool_idx0] = max_pool(temp_output0,size(temp_output0),pool_size0,pool_stride0);
        
        % 2nd convlayer - tanh
        % output0 -> layer1_input = 12*12*20*500
        % convolution
        % temp_output1 = 8*8*50*500
        % maxpooling
        % output1 = 4*4*50*500
        layer1_input = reshape(output0,image_shape1);
        [temp_output1] = convlayer(layer1_input,image_shape1,filter_shape1,conv_stride1,w1,b1);
        [output1,pool_idx1] = max_pool(temp_output1,size(temp_output1),pool_size1,pool_stride1);
        
        % hiddenlayer - tanh
        % output1 -> layer2_input = 800*500
        % output2 = 500*500
        ...
        [w1,b1,delta1] = c_h_pool_updates(learning_rate,filter_shape1,conv_stride1,w1,b1,layer1_input,output1,w2,delta2,'tanh',pool_size1,pool_stride1,pool_idx1);
        [w0,b0,delta0] = c_c_pool_updates(learning_rate,filter_shape0,conv_stride0,w0,b0,layer0_input,output0,filter_shape1,w1,delta1,'tanh',pool_size0,pool_stride0,pool_idx0);
        % validation check
